...
class Node:
    def __init__(self, x, y, theta):
        ...
        self.box = Polygon([[x + self.c*lf - self.s*wc, y + self.s*lf + self.c*wc],
    ...
    def nextNode(self, forward, steer):
        ...
        dtheta = thetastep * forward * steer
        d = dstep * forward
        xnext = self.x + d * np.cos(self.theta + dtheta/2)*np.sinc(dtheta/np.pi/2)
        ynext = self.y + d * np.sin(self.theta + dtheta/2)*np.sinc(dtheta/np.pi/2)
        thetanext = self.theta + dtheta
        # Create the child node.
        child = Node(xnext, ynext, thetanext)

        # Set the parent relationship.
        child.cost = self.cost + cstep * abs(forward) + csteer * (steer != self.steer) + creverse * (self.forward != forward)
        ...
        for forward in [-1, 1]:
            for steer in [-1, 0, 1]:
                ...
                if child.inFreespace() and node.connectsTo(child):
                    ...
                    elif not grid[child.indices()].done and child.cost < grid[child.indices()].cost:
                        onDeck.remove(grid[child.indices()])
                        grid[child.indices()] = child
                        bisect.insort(onDeck, child)
